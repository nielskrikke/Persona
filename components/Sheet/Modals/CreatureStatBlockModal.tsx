...
interface CreatureStatBlockModalProps {
    creature: CreatureDetail | null;
    onClose: () => void;
    variant?: 'familiar' | 'wildshape' | 'polymorph';
}

const CreatureStatBlockModal: React.FC<CreatureStatBlockModalProps> = ({ creature, onClose, variant = 'familiar' }) => {
    if (!creature) return null;

    const formatSpeed = (b: CreatureDetail) => {
        let s = b.speed;
    ...
    const borderColor = variant === 'wildshape' ? 'border-dnd-gold' : variant === 'polymorph' ? 'border-purple-500' : 'border-pink-500';
    const textColor = variant === 'wildshape' ? 'text-dnd-gold' : variant === 'polymorph' ? 'text-purple-400' : 'text-pink-400';
        ...
        <div className="fixed inset-0 bg-black/90 z-[700] flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                ...
                <div className="bg-[#121316] p-6 border-b border-gray-700">
                    <h2 className="text-3xl font-serif text-white mb-1">{creature.name}</h2>
                    <div className="text-sm text-gray-400 italic capitalize">{creature.size} {creature.type}, Unaligned</div>
                </div>
                ...
                <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar space-y-6">
                    ...
                    <div className="space-y-1 text-sm text-gray-300">
                        <div className="flex justify-between border-b border-gray-700 pb-1">
                            ...
                            <span className="text-white font-bold">{creature.ac}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-700 pb-1">
                            <span className="font-bold text-gray-500 uppercase text-xs">Hit Points</span>
                            <span className="text-white font-bold">{creature.hp} <span className="text-gray-500 font-normal">({creature.hit_dice})</span></span>
                        </div>
                        <div className="flex justify-between border-b border-gray-700 pb-1">
                            <span className="font-bold text-gray-500 uppercase text-xs">Speed</span>
                            <span className="text-white font-bold">{formatSpeed(creature)}</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-6 gap-2 text-center bg-gray-800/50 p-3 rounded border border-gray-700">
                        {[
                            { l: 'STR', v: creature.str },
                            { l: 'DEX', v: creature.dex },
                            { l: 'CON', v: creature.con },
                            { l: 'INT', v: creature.int },
                            { l: 'WIS', v: creature.wis },
                            { l: 'CHA', v: creature.cha },
                        ].map(stat => (
                            <div key={stat.l}>
...
export default CreatureStatBlockModal;
